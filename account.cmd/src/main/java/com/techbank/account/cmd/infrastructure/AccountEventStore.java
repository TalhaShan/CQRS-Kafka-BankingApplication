...
public class AccountEventStore implements EventStore {
    ...
    @Override
    public void saveEvents(String aggregateId, Iterable<BaseEvents> events, int expectedVersion) {
    ...
    @Override
    public List<BaseEvents> getEvents(String aggregateId) {
        var eventStream = eventStoreRepository.findByAggregateIdentifier(aggregateId);

        if(eventStream==null || eventStream.isEmpty()){
            throw new AggregateNotFoundException("Incorrect account Id provided");
        }
//eventStream.stream().map(x->x.getEventData()).collect(Collectors.toList());
    ...
    public List<String> getAggregateIds() {
        ...
        if(eventStream==null || eventStream.isEmpty()){
            throw new IllegalStateException("Cannot retrieve stream from event store");
        }
